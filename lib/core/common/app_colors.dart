import 'package:flutter/material.dart';

/// Centralizing application colors
class AppColors {
  AppColors._();

  ///Raw
  static const Color white = Color(0xFFFFFFFF);
  static const Color transparent = Color(0xFFFFFF);
  static const Color blur = Color(0x40FFFFFF);
  static const Color grey = Color(0xFF56575C);
  static const Color lightGrey = Color(0xFFC2C2C2);
  static const Color darkGrey = Color(0xFF292929);
  static const Color darkGrey2 = Color(0xFFE5E5E5);
  static const Color darkGrey50 = Color(0xE1292929);
  static const Color red = Color(0xFFE41D1D);
  static const Color blue = Color(0xFF0044C9);
  static const Color green = Color(0xFF207A18);

  /// Light
  static const Color primaryColorLight = Color(0xFF0E1017);
  static const Color accentColorLight = Color(0xFFFFCE03);

  static const Color lightFontColor = white;
  static const Color lightGreyFontColor = lightGrey;
  static const Color backgroundColorLight = primaryColorLight;
  static const Color textLight = white;
  static const Color darken_shadow = Color(0x652B2A2A);
  static const Color greenColorButton = Color(0xFF00CB76);
  static const Color lightColor = Color(0xFFF3E19B);

  /// Dark

  /// Not theme related

  static const Color greenColor = Color(0xFF00ccb1);

  static const Color bgColor = Color(0xFFfafdff);

  static const Color blueFontColor = Color(0xFF3f62ac);
  static const Color lightBlueColor = Color(0xFFe9f0fa);
  static const Color regularFontColor = Color(0xFF000000);
  static const Color facebookColor = Color(0xFFf4d6cac);
  static const Color termsBackgroundColor = Color(0xFFebf5f7);
  static const Color backgroundGradient1 = Color(0xFF202025);
  static const Color backgroundGradient2 = Color(0xFF393838);
  static const Color backgroundGradient3 = Color(0xFF202025);
  static const Color white_text = Color(0xFFFFFFFF);
  static const Color black_text = Color(0xFF000000);
  static const Color text_gray = Color(0xFF999999);
  static const Color text_gray_c = Color(0xFFcccccc);
  static const Color dark_button_text = Color(0xFFF2F2F2);
  static const Color dark_unselected_item_color = Color(0xFF4D4D4D);
  static const Color disabledTextColor = Color(0xFFa1a1a1);
  static const Color darkBrownColor = Color(0xFF343332);
  static const Color disabledColor = Colors.black87;
  static const Color canvasColor = Colors.transparent;
  static const Color toastTextColor = Colors.white;
  static const Color linearCardTrainee1Color = Color(0xFF4F5055);
  static const Color linearCardTrainee2Color =Color(0xFF43454A);
  static const Color linearCardTrainee3Color = Color(0xFF2F3037);
  static const Color linearCardTrainee4Color =Color(0xFF1D1F26);
  static const Color unselectedColor =Color(0xFFFFF5CD);

}
