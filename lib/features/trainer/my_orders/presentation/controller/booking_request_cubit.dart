...
class BookingRequestCubit extends Cubit<BookingRequestState> {
  ...
  Future approveBookingRequest(BuildContext context,int bookingRequestId) async {
    emit(ApproveBookingRequestLoading());
    final res = await _bookingRequestRepo.approveBookingRequest(bookingRequestId);
    res.fold(
          (err) {
        print(err);
        Toast.show(err);
  ...
  Future rejectBookingRequest(BuildContext context,int bookingRequestId) async {
    emit(RejectBookingRequestLoading());
    final res = await _bookingRequestRepo.rejectBookingRequest(bookingRequestId);
    res.fold(
          (err) {
        print(err);
        Toast.show(err);
